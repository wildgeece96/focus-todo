...
```bash
git clone https://github.com/wildgeece96/bout.git
cd bout
./scripts/install.sh
```

`/Applications/Bout.app` にインストールされます。
...
```bash
# イベントを監視（バックグラウンド向け）
./scripts/watch-events.sh --since now --types phase_completed,timer_paused --watch-messages
```
